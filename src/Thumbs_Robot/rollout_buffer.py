...
class RolloutBuffer:
    ...
    def insert(self, state: torch.Tensor, action: torch.Tensor, reward: float, done: bool, log_prob: torch.Tensor, value: torch.Tensor):
        """
    ...
    def compute_advantages(self, next_value: torch.Tensor, next_done: bool, gamma: float = 0.99, gae_lambda: float = 0.95):
        """
        Calculate GAE advantages:
        delta_t = r_t + gamma * (1 - d_t) * V(s_t+1) - V(s_t)
        A_t = delta_t + gamma * lambda * (1 - d_t) * A_t+1
        """
        last_gae = 0.0
        ...
        # Normalize advantages for learning stability
